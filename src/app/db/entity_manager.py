...
class EntityManager:
   ...
   def _connect(self):
      ...
      try:
         config_connect = {k:v for k,v in config.db.items() if k != "database"}
         logging.debug("Using config for db connexion:  {}".format(config_connect))
         self.conn = mysql.connector.connect(**config_connect, auth_plugin='mysql_native_password')
